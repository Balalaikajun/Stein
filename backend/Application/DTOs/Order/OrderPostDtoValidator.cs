                ...
                switch (order.Type)
                {
                    case OrderTypes.Enrollment or
                        ...
                        if (order.GroupTo == null)
                            context.AddFailure(
                                $"Поле {nameof(order.GroupTo)} обязательно для приказов типа {order.Type}");
                        if (order.GroupFrom != null)
                            context.AddFailure(
                                $"Поле {nameof(order.GroupFrom)} закрыто для приказов типа {order.Type}");
                        break;
                    ...
                    case OrderTypes.TransferFromOtherInstitution or
                        ...
                        if (order.GroupFrom == null)
                            context.AddFailure(
                                $"Поле {nameof(order.GroupFrom)} обязательно для приказов типа {order.Type}");
                        if (order.GroupTo != null)
